...
最后生成候选方案，方案请详细展开你的思路，解释你对题意的理解以及具体的计算步骤，并在最后单独一行写上最终答案，要求格式严格为：
FINAL ANSWER: <纯数字>
例如：
FINAL ANSWER: 15
...
推理过程：
{candidate}
注意：候选方案要求在最后一行严格输出格式为 'FINAL ANSWER: <纯数字>'。
只输出一个数值（例如：0.85）："""
            try:
...
"""
    ...
    def solve(self, question: str, max_steps: int = 6, n_samples_per_step: int = 3,
        ...
        revision_threshold 参数用于判断候选方案分数是否足够优秀，不足时触发修正逻辑。
        """
        infos = []  # 记录每轮信息
        ...
        for step in range(max_steps):
            ...
            for state in states:
                ...
                # 如果只希望生成单个候选方案，可以将 n_samples 设置为 1
            ...
            # Revision阶段：仅对分数低于 revision_threshold 的候选方案进行修正
            for i, ref in zip(idxs, refutations):
                cand = new_candidates[i]
                if candidate_scores[i] < revision_threshold:
