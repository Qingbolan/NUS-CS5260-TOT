...
class TreeOfThoughts:
    ...
    def chat_with_gpt(self, prompt: str, n: int = 1, stop: str = None) -> List[str]:
        """Get completions from the GPT model."""
    ...
    def extract_final_answer(self, thought: str) -> str:
        """
        Extract the final answer from the thought using a designated marker.
        First, try to extract text following 'FINAL ANSWER:'.
        If not found, fallback to the '####' marker.
        """
    ...
    def generate_thoughts(self, question: str, current_thought: str = "", n_samples: int = 3) -> List[str]:
        """
        Generate multiple possible reasoning paths.
        The prompt instructs the model to:
         - Construct a logical tree.
         - Perform sequential, quantitative, and branching reasoning.
         - Clearly distinguish intermediate steps from the final answer.
         - Output one final answer on a separate line in the exact format:
        FINAL ANSWER: <your final answer>
        """
        prompt = f"""You are a highly skilled mathematical problem solver. In this task, perform Tree-of-Thoughts (TOT) reasoning with the following steps:

        1. **Construct a Logical Tree:**
        - Map all logical relationships and dependencies.
        - Clearly identify base entities, their relationships, and any intermediate calculations.

        2. **Time and Sequential Reasoning:**
        - Identify temporal reference points and ensure the correct sequence of events.
        - Clearly indicate if parts of the work occur before or within a specified time period.
        ...
        - Explore multiple reasoning branches and label them clearly (e.g., Branch 1, Branch 2, etc.).

        5. **Final Answer Extraction:**
        - On a separate line at the end, output your final answer exactly in the format:
                FINAL ANSWER: <your final answer>
        - Ensure that this final answer fully reflects all necessary calculations, even if some work occurred outside a given time frame.

        Now, solve the following problem in a TOT manner:
        Question: {question}
        Current Thought (if any): {current_thought}

        Provide multiple reasoning branches."""
        
        return self.chat_with_gpt(prompt, n=n_samples)

    def evaluate_thought(self, question: str, thought: str, cache: bool = True) -> float:
        """
        Evaluate the reasoning process focusing on:
         - Logical structure,
         - Calculation accuracy,
         - Quality of branching,
         - Final answer extraction.
         
        The evaluator's response should be a single float between 0 and 1.
        """
        if cache and thought in self.thought_cache:
            return self.thought_cache[thought]
        
        # If no final answer marker is found, return a low score immediately.
        if "FINAL ANSWER:" not in thought and "####" not in thought:
            return 0.0
        
        prompt = f"""You are a mathematical solution evaluator. Evaluate the following reasoning process based on these criteria:
            ...
            - Is the final answer clearly marked on a separate line using "FINAL ANSWER:" (or "####")?
            - Is the final answer unambiguous and consistent with the reasoning process?
        ...
        try:
            ...
            # Use regex to extract the first numeric value from the response.
    ...
    def self_supervised_validation(self, thought: str) -> bool:
        """
        Perform a self-supervised check to ensure that the final answer extracted from the thought 
        is consistent with the provided reasoning process.
        Returns True if the final answer is fully consistent; otherwise, False.
        """
        final_answer = self.extract_final_answer(thought)
        if not final_answer:
            return False
        
        validation_prompt = f"""Review the following reasoning process. Is the final answer (the line starting with "FINAL ANSWER:" or "####") fully consistent with all the calculations and logical steps provided?
    ...
    def solve(self, question: str, max_steps: int = 8, n_samples_per_step: int = 3, 
        ...
        for step in range(max_steps):
            ...
            for current_text, current_score in states:
                # Greedy pruning: skip paths with very low scores relative to the current best.
                ...
                for thought in generated_thoughts:
                    ...
                    if "FINAL ANSWER:" in combined or "####" in combined:
                        if not self.self_supervised_validation(combined):
                            score *= 0.9  # Penalize if self-validation fails.
                            
                    # Update the best solution if a final answer is found and the score improves.
                    if (("FINAL ANSWER:" in combined or "####" in combined) and score > best_score):
                        best_score = score
            ...
            # Break early if no promising states remain.
