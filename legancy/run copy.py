...
# The `TreeOfThoughts` class is a solver that uses the OpenAI GPT model to generate and evaluate
# multiple possible next steps in reasoning to solve a given problem.
...
class TreeOfThoughts:
    ...
    def generate_thoughts(self, question: str, current_thought: str = "", n_samples: int = 3) -> List[str]:
        ...
        # pass
        prompt = f"""Let's solve this problem by first generating 5 different possible approaches.

Question: {question}
Current Progress: {current_thought}
...
Generate {n_samples} different approaches:
Approach 1:
[Your detailed plan here]

[After generating plans, I will analyze them to select the most promising one]

Please provide your next step in the reasoning process based on the above analysis:"""
    ...
    def evaluate_thought(self, question: str, thought: str, cache: bool = True) -> float:
        ...
        # pass
        prompt = f"""Please analyze the following solution approaches and vote for the most promising one.

Question: {question}
Proposed Solution: {thought}

Evaluate based on:
1. Logical coherence and clarity
2. Progress toward the solution
3. Potential for success
4. Alignment with problem requirements

After analyzing the approach, rate its likelihood of success from 0 to 1.
Output only the numeric rating:"""
    ...
    def solve(self, question: str, max_steps: int = 8, n_samples_per_step: int = 3, 
        ...
        # pass
        states = [("Let's approach this question by first considering multiple possible strategies:", 0.0)]
        
        # First generate 5 different plans
        plans_prompt = f"""Generate 5 different plans for solving this problem:
        Question: {question}
        ...
        Label each plan clearly as Plan 1, Plan 2, etc."""
        
        plans = self.chat_with_gpt(plans_prompt, n=1)[0]
        initial_thought = f"Generated Plans:\n{plans}\n\nLet's proceed with the most promising approach:"
        states = [(initial_thought, 0.0)]
